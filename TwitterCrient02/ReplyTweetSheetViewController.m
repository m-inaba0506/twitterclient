//
//  ReplyTweetSheetViewController.m
//  TwitterCrient02
//
...
@interface ReplyTweetSheetViewController ()

@end

@implementation ReplyTweetSheetViewController
...
- (void)viewDidLoad
{
    ...
    NSString *initialText = [NSString stringWithFormat:@"@%@ ",self.name];
    _tweetTextView.text = initialText;
    // ツールバーの作成
...
- (IBAction)tweetAction:(id)sender {
    ...
    ^(BOOL granted, NSError *error) {
        ...
        if (granted) {
            ...
            NSDictionary *params = @{@"status" : tweetString,@"in_reply_to_status_id" : self.idStr};
